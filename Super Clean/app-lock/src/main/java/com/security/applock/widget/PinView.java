...
public class PinView extends AppCompatEditText {
    ...
    private OnCheckPinCode onCheckPinCode;
    ...
    public PinView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        addTextChangedListener(new TextWatcher() {
            ...
            public void onTextChanged(CharSequence s, int start, int before, int count) {
                if (getItemCount() == getText().length()) {
                    onCheckPinCode.onCheck(getText().toString());
                }
            }
    ...
    private void drawPinView(Canvas canvas) {
        ...
        for (int i = 0; i < mPinItemCount; i++) {
            boolean highlight = highlightIdx == i;
        ...
        if (getText().length() != mPinItemCount && mViewType == VIEW_TYPE_RECTANGLE) {
    ...
    public OnCheckPinCode getOnCheckPinCode() {
        return onCheckPinCode;
    }

    public void setOnCheckPinCode(OnCheckPinCode onCheckPinCode) {
        this.onCheckPinCode = onCheckPinCode;
    }
    ...
    static class DefaultMovementMethod implements MovementMethod {
